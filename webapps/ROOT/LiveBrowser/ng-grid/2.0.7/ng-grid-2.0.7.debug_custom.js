...
// ESPRESSO LOGIC PATCH -- see https://github.com/NewMediaRoc/ng-grid/commit/5e95ee396b102b71923855c36106e4285ae0f8c3
        ...
        compile: function() {
            return {
                pre: function($scope, iElement) {
                    var html;
                    var safePath = 'row.entity.' + $scope.col.field; //safest path, allows for infinte nesting
                    if (safePath.match(/@metadata/)) {
                    	safePath = 'row.entity["' + $scope.col.field + '"]'; //disallowed character causes error, safe b/c @metadata nest not needed
                    }
                    var cellTemplate = $scope.col.cellTemplate.replace(COL_FIELD, safePath);

                    if ($scope.col.enableCellEdit) {
                        html =  $scope.col.cellEditTemplate;
                        html = html.replace(DISPLAY_CELL_TEMPLATE, cellTemplate);
                        html = html.replace(EDITABLE_CELL_TEMPLATE, $scope.col.editableCellTemplate.replace(COL_FIELD, safePath));
                    } else {
                        html = cellTemplate;
                    }
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("checkboxCellTemplate.html",
    "<div class=\"ngSelectionCell\"><input ng-trim='false' tabindex=\"-1\" class=\"ngSelectionCheckbox\" type=\"checkbox\" ng-checked=\"row.selected\" /></div>"
  );

  $templateCache.put("checkboxHeaderTemplate.html",
    "<input ng-trim='false' class=\"ngSelectionHeader\" type=\"checkbox\" ng-show=\"multiSelect\" ng-model=\"allSelected\" ng-change=\"toggleSelectAll(allSelected, true)\"/>"
  );

  $templateCache.put("editableCellTemplate.html",
    "<input ng-trim='false' ng-class=\"'colt' + col.index\" ng-input=\"COL_FIELD\" ng-model=\"COL_FIELD\" />"
  );
  ...
  $templateCache.put("footerTemplate.html",
    ...
    "            <input ng-trim='false' class=\"ngPagerCurrent\" min=\"1\" max=\"{{maxPages()}}\" type=\"number\" style=\"width:50px; height: 24px; margin-top: 1px; padding: 0 4px;\" ng-model=\"pagingOptions.currentPage\"/>" +
  ...
  $templateCache.put("headerRowTemplate.html",
    "<div ng-style=\"{ height: col.headerRowHeight }\" ng-repeat=\"col in columns\" ng-class=\"col.colIndex()\" class=\"ngHeaderCell\">" +
  ...
  $templateCache.put("menuTemplate.html",
    ...
    "        <input ng-trim='false' placeholder=\"{{i18n.ngSearchPlaceHolder}}\" type=\"text\" ng-model=\"filterText\"/>" +
    "    </div>" +
    "    <div ng-show=\"showColumnMenu\">" +
    "        <span class=\"ngMenuText\">{{i18n.ngMenuText}}</span>" +
    "        <ul class=\"ngColList\">" +
    "            <li class=\"ngColListItem\" ng-repeat=\"col in columns | ngColumns\">" +
    "                <label><input ng-trim='false' ng-disabled=\"col.pinned\" type=\"checkbox\" class=\"ngColListCheckbox\" ng-model=\"col.visible\"/>{{col.displayName}}</label>" +
  ...
  $templateCache.put("rowTemplate.html",
    "<div ng-style=\"{ 'cursor': row.cursor }\" ng-repeat=\"col in columns\" ng-class=\"col.colIndex()\" class=\"ngCell {{col.cellClass}}\">" +
