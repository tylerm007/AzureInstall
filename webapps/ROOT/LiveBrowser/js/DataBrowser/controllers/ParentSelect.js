...
//		$scope.selectedRole = espresso.util.getFirstProperty(childTableInfo.parentsByName);
		...
		$scope.buildSearchFilter = function() {
			...
			for (var i = 0; i < $scope.searchCriteria.length; i++) {
				...
				var fragment = "";
				switch($scope.searchCriteria[i].col.generic_type) {
					case 'text':
						fragment = $scope.searchCriteria[i].col.name + " LIKE '%" + encodeURIComponent($scope.searchCriteria[i].colValue) + "%'";
						break;
					case 'number':
						if (typeof $scope.searchCriteria[i].colValue == 'number') {
							$scope.searchCriteria[i].colValue = $scope.searchCriteria[i].colValue.toString();
						}
						var char1 = $scope.searchCriteria[i].colValue.charAt(0);
						if (char1 == '>' || char1 == '<') {
							fragment = $scope.searchCriteria[i].col.name + $scope.searchCriteria[i].colValue;
						}
						else {
							fragment = $scope.searchCriteria[i].col.name + "=" + $scope.searchCriteria[i].colValue;
						}
