(function() {
  ...
  app.directive("quickDatepicker", [
    'ngQuickDateDefaults', '$filter', '$sce', '$rootScope', function(ngQuickDateDefaults, $filter, $sce, $rootScope) {
      return {
        restrict: "E",
        require: "?ngModel",
        scope: {
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
          var dateToString, datepickerClicked, datesAreEqual, datesAreEqualToMinute, getDaysInMonth, 
          initialize, parseDateString, refreshView, setCalendarDate, setConfigOptions, setInputFieldValues, setupCalendarView, stringToDate;
          scope.$parent.$watch('row', function () {
        	 initialize();
          });
          initialize = function() {
            setConfigOptions();
          ...
          window.document.addEventListener('click', function(event) {
        	  //patched: LB was triggering clicks to the main grid currently active row which subverted the update;
            if (scope.calendarShown && !datepickerClicked && !angular.element(event.target).hasClass('selected')) {
              scope.toggleCalendar(false);
              scope.$apply();
            }
          ...
          refreshView = function() {
            var date;
            date = ngModelCtrl.$modelValue ? new moment(ngModelCtrl.$modelValue).toDate() : null;
            setupCalendarView();
          ...
          setCalendarDate = function(val) {
            ...
            d.setDate(2);
            $rootScope.$emit('ngGridEventEndCellEdit');
          ...
          scope.toggleCalendar = function(show) {
            ...
            if (scope.calendarShown) {
            	setTimeout(function () {
            		var $popup = $('.quickdate-popup.open');
            		var $body = $('body');
            		var $mainGrid = $popup.closest('#leftSide');
            		if ($mainGrid.length) {
            			var offset = $popup.offset();
            			var repositionLongitude = $body.height() * .75;
            			var repositionLatitude = $mainGrid.width() * .75;
            			console.log(repositionLongitude, repositionLatitude, offset);
            			if (offset.top > repositionLongitude) {
            				$popup.css({'top':'-400px'});
            			}
            			if (offset.left > repositionLatitude) {
            				$popup.css({'left':'-75px'});
            			}
            		}
            	}, 50);
          	  $('#childCollections').css('z-index', '999');
            }
            else {
          	  $('#childCollections').css('z-index', '0');
            }
